#!/bin/bash
# require.sh -- checked BEFORE the build when WITH_FIREFOX=true: APK(s) present (each module is guarded
...
# vendor/lineage/prebuilts/firefox. Without one (ether 20.0) the module is a device-tree module the
...
_dir="$AOSP/vendor/lineage/prebuilts/firefox"
_dev="$AOSP/device/${DEVICE:-}/firefox"
[ -f "$_dev/Android.mk" ] && _dir="$_dev"
app_require firefox "$_dir" Firefox
